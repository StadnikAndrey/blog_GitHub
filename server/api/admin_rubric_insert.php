...
if ($token) {
	...
	if ((int) $tokenData['exp'] >= time()) {
		...
		// проверка на наличие добавляемой рубрики
		$rubric = Articles::getRubricForName(trim($_POST['name']));
		if ($rubric) {
		    $error[] = 'Такая рубрика уже существует!';
		}     	 

     	if (empty($error)) {
     		$insert_id = Utils::insertDataTable($data, 'rubrics');
     		$response = ["res" => true];
     	}else{
